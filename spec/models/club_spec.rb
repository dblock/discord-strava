...
describe Club do
  ...
  context 'sync_last_strava_activity!', vcr: { allow_playback_repeats: true, cassette_name: 'strava/club_sync_last_strava_activity' } do
    ...
    it 'disconnects club on auth failure' do
      ...
      expect(club.team.discord_client).to receive(:chat_postMessage).with(
        club.to_discord.merge(
          text: 'There was an authorization problem. Please reconnect the club via /strada clubs.',
    ...
    it 'disables sync on 404' do
      ...
      expect(club.team.discord_client).to receive(:chat_postMessage).with(
        club.to_discord.merge(
          text: 'Your club can no longer be found on Strava. Please disconnect and reconnect it via /strada clubs.',
          channel: club.channel_id,
          as_user: true
        )
      ).and_return('ts' => 1)
    ...
    context 'after an initial sync' do
      ...
      context 'with a new activity' do
        ...
        it 'syncs and brags' do
          expect_any_instance_of(Discord::Web::Client).to receive(:chat_postMessage).once
          club.sync_and_brag!
        end
      end
    end
