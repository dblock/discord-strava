...
describe DiscordStrava::Commands::Disconnect do
  ...
  context 'disconnect' do
    it 'requires a subscription' do
      expect(message: "#{DiscordRubyBot.config.user} disconnect").to respond_with_discord_message(team.trial_message)
    end
    context 'subscribed team' do
      ...
      context 'connected user' do
        ...
        it 'disconnects a user' do
          ...
          message_hook.call(client, Hashie::Mash.new(channel: 'channel', user: DiscordRubyBot.config.user, text: "#{DiscordRubyBot.config.user} disconnect"))
          user.reload
